...
Объектные переменные являются лишь указателями на объекты в хипе, и сами как и локальные переменные хранятся в стэке.
...
Ссылка может escape только в случае если мы вовзвращаем ссылку на объект. Следовательно нужно избегать отдавания ссылок на существующие объекты. 
В случае если ссылка на объект отдается для итерации по содержимому объекта, то можно было бы сделать итерируемым сам главыный объек, который отдавал ссылку. Однако итератор позволяет удалять элементы коллекции.
Лучшим решением может быть отдать новый экземпляр коллекции проинициилизированный существующей коллекцией, то есть создать копию возвращаемого объекта (коллекции). Но у этого решения есть недостаток в том, что клиент получивший копию может решить что изменяя ее он также изменяет исходную коллекцию.
