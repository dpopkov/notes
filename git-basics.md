...
2. Копировать SSH link репозитория
3. В локальной директории проекта выполнить `git init`
4. Прописать скопированный адрес нового репозитория: `git remote add origin COPIED_GIT_LINK`
5. Проверить адрес: `git remote -v`
6. Выполнить команду для переключения ветки: `git checkout -b master`
7. Выполнить команду для синхронизации: `git fetch`
