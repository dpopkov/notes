...
Методы: readLine() : String, readPassword() : char[], format(), printf().
...
java.nio.file.Path является интерфейсом. Для получения Path можно использовать java.nio.file.Path метод get(String). При получении объекта Path на Windows это будет объект класса sun.nio.fs.WindowsPath.
