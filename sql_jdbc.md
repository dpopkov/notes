...
Подзапрос - это вложенная команда SELECT, заключенная в круглые скобки и являющийся частью более общего запроса. Используется для получения значений или набора данных, передаваемого в основной запрос.
...
UNION - операция объединения двух наборов строк, возвращаемых SQL запросами. Оба запроса дожлны возвращать одинаковое количество столбцов и соответствующие по порядку столбцы должны иметь совместимые типы данных. При объединении нескольких запросов подряд результат последовательно вычисляется слева направо.


15  Что такое group by?<a name="q15"></a>
-----------------------

GROUP BY - операция группировки результата запроса по значениям указанных столбцов. Первоначальная общая выборка строк разделяется на отдельные группы строк, для которых столбцы указанные в GROUP BY имеют одинаковые значения. При этом итоговая выборка должна содержать только те столбцы, которые участвововали в группировке, а также результаты агрегирующих функций, таких как count(), sum(), average().
