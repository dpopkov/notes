...
* [1.1  Что такое естественный ключ?](#q0101)
* [1.2  Что такое суррогатный ключ?](#q0102)
...
* [3.0.2 Что такое транзитивная зависимость](#q030002)
...
1.1  Что такое естественный ключ? <a name="q0101"></a>
---------------------------------
Естественный ключ - набор атрибутов описываемой сущности, уникально ее идентифицирующий (например номер паспорта для человека).


1.2  Что такое суррогатный ключ? <a name="q0102"></a>
--------------------------------

Суррогатный ключ - автоматически сгенерированное поле, никак не связанное с информационным содержанием записи. Обычно в роле СК выступает автоинкрементное поле типа integer.
...
3.0.2 Что такое транзитивная зависимость<a name="q030002"></a>
------------------------------------------
Если для атрибутов A, B, C некоторого отношения существуют функциональные зависимости A -> B, B -> C, то говорят, что атрибут C связан транзитивной зависимостью с атрибутом A через атрибут B (при этом атрибут A не должен функционально зависеть ни от атрибута B, ни от атрибута C).

{студент} -> {{предмет} -> {учебник}}
...
2НФ относится только к случаям если в схеме есть составной Первичный Ключ. Если Первичный ключь простой, то отношение уже находится во 2НФ.
...
Курсор БД это управляющая структура которая позволяет последовательно получить доступ к строкам в результирующим наборе. 
Для результирующего набора строк курсор является аналогом итератора, то есть он указывает на отдельную строку в результирующем наборе строк. 
SQL курсоры могут выбирать несколько строк за одну операцию, то есть курсор является способом обрабатывать по частям большие наборы строк. Курсор также может перемещаться в обратном направлении. 
Курсор может быть входным параметром функции и функция может также возвращать ссылку на курсор.
Синтаксис для работы с курсором в SQL и процедурном языке (PL/pgSQL) имеет различия.
...
Подзапрос - это вложенная команда SELECT, заключенная в круглые скобки и являющийся частью более общего запроса. Подзапрос возвращает значения или набор данных, который используется в основном запросе.

Подзапрос может присутствовать в месте, где по смыслу может находиться таблица. Например подзапрос может возвращать набор строк, который будет интерпретироваться как таблица в части FROM основного  запроса.

Подзапрос также может быть скалярным, то есть возвращать одну строку и один столбец, с которым можно сравнивать другие значения. Например подзапрос может возвращать скалярное значение, которое будет частью выражения фильтрующего выборку с помощью условия WHERE или HAVING.
...
UNION - операция объединения двух наборов строк, возвращаемых SQL запросами. Оба исходных запроса дожлны возвращать одинаковое количество столбцов и соответствующие по порядку столбцы должны иметь совместимые типы данных. Результат содержит все строки из исходных наборов и не содержит дубликатов. При объединении нескольких запросов подряд результат последовательно вычисляется слева направо.


15  Что такое group by?<a name="q15"></a>
-----------------------

Когда вы добавляете GROUP BY, то указываете столбцы в логической таблице сформированной частями запроса FROM и WHERE, которые вы хотите использовать как определения для групп строк. Строки имеющие одинаковые значения в списк столбцов собираются в группы. Вы можете использовать столбцы указанные в GROUP BY в части выражений SELECT, и вы можете использовать агрегирующие функции для выполнения вычислений в пределах группы.

GROUP BY - операция группировки результата запроса по значениям указанных столбцов. 
Первоначальная общая выборка строк, полученная из исходных таблиц и отфильтрованная по условию WHERE, разделяется на отдельные группы строк, для которых столбцы указанные в GROUP BY имеют одинаковые значения. При этом итоговая выборка может содержать только те столбцы, которые участвововали в группировке, а также результаты агрегирующих функций, таких как count(), sum(), avg().
