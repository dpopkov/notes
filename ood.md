...
Java code conventions - это соглашение принятое компанией Sun Microsystems для поддержания чистоты и единообразия исходного кода. Это соглашение как правило берется за основу и незначительно дополняются.
...
Cohesion - связность или степень того, насколько хорошо все методы класса или все фрагменты метода соответствуют главной цели, - иначе говоря, насколько сфокусирован класс на той задаче, которую он решает.
...
Для решения DIP может использоваться Шаблон Фабричного Метода, который создает интерфейс для создания объектов, но созданием конкретных объектов занимаются уже подклассы, реализующие этот интерфейс. При этом фабричный метод возвращает абстрактный тип или интерфейс, который служит защитой клиентского кода от зависимости от конкретных классов, создаваемых фабричным методом.
