...
* [20.1 - Решение DIP](#q2001)
* [21 - Расскажите, что такое автоматическое тестирование](#q21)
* [22 - Как в Java осуществляется автоматическое тестирование](#q22)
* [23 - Что такое JUnit? Как использовать для тестирования?](#q23)
* [24 - Что такое функциональное тестирование и чем отличается от модульного](#q24)
* [25 - Расскажите про принцип TDD](#q25)
* [26 - Расскажите про принцип BDD](#q26)
* [27 - Что такое тестирование черным ящиком](#q27)
...
4 - Что такое Java code conventions?<a name="q04"></a>
------------------------------------

Java code conventions - это соглашение принятое компанией Sun Microsystems для поддержания чистоты и единообразия исходного кода. Это соглашение как правило берется за основу и незначительно дополняются для стиля компании.
Соглашения важны так как значительная часть времени приходится на чтение и поддержку кода и очень часто это делает не автор, а другие программисты. Соглашения помогают улучшить читаемость кода, позволяя быстрее и лучше понимать чужой код.
Соглашения включают правила по наименованию файлов исходного кода, классов и методов, переменных и констант, по размещению комментариев, использованию пробелов и пустых строк, порядка элементов класса и т.п.

5 - Расскажите про принципы составления Java класса<a name="q05"></a>
---------------------------------------------------
...
Cohesion - внутренняя связность, сплоченность или степень того, насколько хорошо все методы класса или все фрагменты метода соответствуют главной цели, - иначе говоря, насколько сфокусирован класс на той задаче, которую он решает.
...
6.3 - Что такое Loose Coupling<a name="q0603"></a>
------------------------------

Loose Coupling (Слабая сопряженность) это сопряжение между компонентами, при котором каждый из компонентов может взаимодействовать с другими, но имеет о них минимальное знание. Например может быть известно только, что компонент имплементирует определенный интерфейс минимальный для конкретного взаимодействия, но не более того.
Слабосопряженный дизайн позволяет строить ОО системы, которые легче переносят изменения потому что в них взаимозависимость между объектами минимизирована.
...
Нарушение SRP понижает связность класса, из-за чего код трудно понимать, трудно использовать, трудно поддерживать и тестировать.
...
При этом OCP не должен применяться против всех возможнох изменений, а только против наиболее вероятных, так как лишние абстракции увеличивают сложность дизайна программы.
...
При наследовании поведение подтипа не должно противоречить поведению базового типа. Мы можем использовать взаимозаменяемые части, но для этого они должны придерживаться некоторого договора или контракта. В случае наследования подклассы обязаны соблюдать контракт базового класса и дополнять, а не замещать поведение базового класса.
...
ISP: клиентский код не должен зависеть от методов, которые он не использует. ISP советует разбивать большие интерфейсы на интерфейсы меньшего размера, которые являются более сфокусированными таким образом, чтобы клиентскому коду были доступны только те методы, которые ему действительно нужны. Такие сокращенные интерфейсы называются Ролевыми. Назначение ISP в сокранении системы слабосопряженной (decoupled) и лучше приспособленной для изменений и рефакторинга.
...
### TODO: Дать Описание примера ATM из ASDPPP


17 - Последствия нарушения ISP<a name="q17"></a>
------------------------------

Если мы определяем большой интерфейс, то в имплементирующих классах возможно появление множества методов-заглушек, которые нужны только ради обеспечения компилируемости кода и по сути ничего не делают, то есть это приводит к лишнему коду.
...
Принцип DIP утверждает, что наиболее гибкими получаются системы, в которых зависимости направлены на абстракции, а не на конкретные имплементации.
...
При традиционном процедурном программировании создается струтура подпрограмм, в которой код верхнего уровня вызывает функции нижнего уровня. При этом возникает структура зависимостей в которой код верхнего уровня зависит от нижнего. 
Это делает код верхнего уровня зависимым от изменений в нижнем. 
DIP переворачивает эту структуру зависимостей таким образом, чтобы оба зависели от абстракции. Вводятся дополнительные интерфейсы, которые имплементируются нижним уровнем, при этом верхний уровень часто владеет теми интерфейсами, которые он использует.
Если DIP применен, то программа имеет ОО дизайн, если нет, то процедурный. Применение DIP необходимо для создания фреймворков, а также необходимо для создания кода устойчивого к изменениям.
...
20.1 - Решение DIP<a name="q2001"></a>
------------------

Для решения DIP может использоваться Шаблон Фабричного Метода, который вводит интерфейс для создания объектов, но созданием конкретных объектов занимаются уже классы, реализующие этот интерфейс. 
При этом фабричный метод возвращает абстрактный тип или интерфейс, предотвращающий зависимость клиентского кода от конкретных классов, объекты которых создаются конкретной реализацией этого абстрактного фабричного метода.



21 - Расскажите, что такое автоматическое тестирование<a name="q21"></a>
------------------------------------------------------

Автоматизированное тестирование - это использование одних программ для тестирования других програм, находящихся в стадии разработки или контроля качества. При этом используются специальные программные средства которые выполненяют тесты и проверяют результаты их выполнения.
Два основных подхода к автоматизации тестирования это тестирование на уровне кода (к которому относится модульное тестирование) и тестирование на уровне пользователя, то есть воспроизведение действий пользователя, проверяющего функциональность программы.


22 - Как в Java осуществляется автоматическое тестирование<a name="q22"></a>
----------------------------------------------------------

Автоматическое тестирование в Java осуществляется с помощью специализированных средств:

Selenium - фреймворк, включающий в себя несколько инструменттов для управления браузерами и записи действий пользователя, который используются для тестирования веб-приложений.

Фрейворки модульного тестирования, основными из которых являются JUnit и TestNG.


23 - Что такое JUnit? Как использовать для тестирования?<a name="q23"></a>
--------------------------------------------------------

JUnit это фрейворк для модульного тестирования, то есть тестирования отдельных участков кода, например методов или классов. 
Тестом является отдельный класс, public методы которого преверяют ожидаемое поведение тестируемого кода. Начиная с 4-й версии тестирующие методы объявляются с помощью аннотаций @Test. 
В каждом тестирующем методе создается начальное состояние, затем вызываются методы тестируемого кода, далее происходит сверка ожидаемых и реальных результатов. Если результаты совпадают, то тест пройден. Таким образом юнит тесты являются разновидностью документации на то, что ожидается в результате выполнения тестируемого кода.


24 - Что такое функциональное тестирование и чем отличается от модульного<a name="q23"></a>
-------------------------------------------------------------------------

Функциональное тестирование - это тестирование проверяющее соответствие программы конкретным пользовательским требованиям по решению задач, ради которых оно создано. То есть это проверка того, что программа делает то, что должна делать, как указано в спецификации. При этом внутреняя структура программы не рассматривается, а проверяется ее работоспособность как черного ящика.

Модульное тестирование - это проверка корректности отдельных частей кода, классов, методов. То есть отдельные модули проверяются изолированно друг от друга, с разными наборами входных данных и ожидаемых результатов, которые должны покрывать возможные ситуации использования конкретного класса или метода. Модульное тестирование является часть процесса написания кода.


25 - Расскажите про принцип TDD<a name="q25"></a>
-------------------------------

TDD - техника разработки, при которой сначала пишется тест на определенный функционал, а затем пишется реализация этого функционала, которая обеспечивает прохождение теста. Потом делается рефакторинг, приводящий новый код к соответствующим стандартам. В результате код не только написан и протестирован, но тесты задают требования к функционалу и показывают пример использования этого функционала.


26 - Расскажите про принцип BDD<a name="q26"></a>
-------------------------------

Behavior-driven delelopment, то есть разработка через поведение, является ответвлением TDD, в котором используется предметно-ориентированный язык, на котором описывается поведение программного продукта и ожидаемые результаты.
После написания сценария на полуформальном предметно-ориентированном языке тесты могут автоматически переводиться в Java и запускаться с помощью JUnit или другого фреймворка. Примеры BDD фрейворков: JBehave, Cucumber.


27 - Что такое тестирование черным ящиком<a name="q27"></a>
-------------------------------------------

При тестировании программы как черного ящика внутренняя структура приложения в расчет не принимается, а значение имеет только функциональность, которую это приложение должно обеспечить. В отличие от этого, при тестировании белого ящика во внимание принимается внутренняя структура, то есть классы и методы.
